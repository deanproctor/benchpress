...
for version in ${versions[@]}; do
	...
	ste start PostgreSQL_9.6.2
        stf test -sv --sdc-version $version --database postgresql://postgres.cluster:5432/default --benchmark-arg=KEEP_TABLE=true --benchmark-arg=DATASET=census $test 
        stf test -sv --sdc-version $version --database postgresql://postgres.cluster:5432/default --benchmark-arg=KEEP_TABLE=true --benchmark-arg=DATASET=sales $test 
